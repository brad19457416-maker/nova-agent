...
class MemoryPalace:
    ...
    def __init__(self, data_dir: str = "./data/memory", 
        ...
        # 宫殿结构索引
        self.structure_index: Dict[str, Any] = {
            "wings": {},
            "total_memories": 0
        }
    ...
    def add_memory(self, wing: str, room: str, hall: str, 
                  content: str, compressed: bool = True) -> str:
        """
        添加记忆到宫殿
        ...
        if hall not in self.structure_index["wings"][wing]["rooms"][room]["halls"]:
            self.structure_index["wings"][wing]["rooms"][room]["halls"][hall] = {
                "memories": []
            }
        ...
        logger.debug(f"Added memory {memory_id}, compressed ratio: {len(compressed_content)/len(content):.2f}")
        return memory_id
    
    def retrieve(self, query: str, hierarchical_filter: bool = True, 
               contradiction_check: bool = True, top_k: int = 10) -> List[Dict]:
        """
        检索相关记忆
    ...
    def _load_content(self, result: Dict) -> str:
        ...
        if "path_closet" in result:
            with open(result["path_closet"], 'r', encoding='utf-8') as f:
                return f.read()
            ...
            if closet_path.exists():
                with open(closet_path, 'r', encoding='utf-8') as f:
                    return f.read()
    ...
    def get_memory_full(self, memory_id: str) -> Dict[str, str]:
        ...
        if len(parts) >= 4:
            ...
            compressed = open(closet_path, 'r', encoding='utf-8').read() if closet_path.exists() else ""
            original = open(drawer_path, 'r', encoding='utf-8').read() if drawer_path.exists() else ""
