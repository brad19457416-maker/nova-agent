...
class ContradictionChecker:
    ...
    def __init__(self, similarity_threshold: float = 0.7, contradiction_threshold: float = 0.5):
    ...
    def filter_contradictions(self, query: str, results: List[Dict]) -> List[Dict]:
        ...
        for i, result_a in enumerate(results):
            ...
            if not is_contradictory:
                ...
                logger.info("Contradiction filtered out from retrieval")
    ...
    def _detect_contradiction(self, text_a: str, text_b: str) -> bool:
        ...
        # 完整检测交给 LLM 在推理阶段处理
        has_negation_a = any(
            word in text_a.lower() for word in ["not", "never", "no", "false", "incorrect"]
        )
        has_negation_b = any(
            word in text_b.lower() for word in ["not", "never", "no", "false", "incorrect"]
        )
