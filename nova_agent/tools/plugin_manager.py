...
class PluginManager:
    ...
    def execute_plugin(self, name: str, parameters: Dict[str, Any], **kwargs) -> PluginResult:
        ...
        if plugin is None:
            return PluginResult(success=False, error=f"Plugin not found or disabled: {name}")

        if not plugin.validate_parameters(parameters):
            return PluginResult(success=False, error=f"Parameter validation failed for {name}")
