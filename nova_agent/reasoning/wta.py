...
class WTASelection:
    ...
    def select(self, blocks: list[dict], query: str) -> list[dict]:
        ...
        sorted_blocks = sorted(
            blocks, key=lambda b: b.get("similarity", b.get("gain", 0)), reverse=True
        )
    ...
    def select_per_cluster(
        ...
        for cluster in clusters:
            ...
            cluster_sorted = sorted(
                cluster_blocks, key=lambda b: b.get("similarity", 0), reverse=True
            )
            selected.extend(cluster_sorted[:top_per_cluster])

        # 如果总数超限，再次 WTA
        if len(selected) > self.max_activate:
            selected = sorted(selected, key=lambda b: b.get("similarity", 0), reverse=True)[
                : self.max_activate
            ]
