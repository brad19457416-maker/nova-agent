...
class BidirectionalAttentionFlow:
    ...
    def reverse_activate(self, 
        ...
        # 提取聚合文本中的关键概念
        new_concepts = self._extract_new_concepts(
            aggregated_text, 
            current_blocks
        )
        ...
        # 检查这些新概念是否需要新增块
        new_blocks = self._generate_new_blocks(
            new_concepts, 
            aggregated_text,
            context
        )
    ...
    def _extract_new_concepts(self, aggregated_text: str, 
                             current_blocks: List[Dict]) -> List[str]:
    ...
    def _generate_new_blocks(self, new_concepts: List[str],
                           aggregated_text: str,
                           context: Dict) -> List[Dict]:
        """为新概念生成新的子任务块"""
        new_blocks = []
