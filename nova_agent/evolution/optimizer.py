...
class StrategyOptimizer:
    ...
    def optimize(self, evaluation, current_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if quality < 0.3:
            ...
            new_config["cumulative_gain_threshold"] = current_config.get("cumulative_gain_threshold", 2.5) + 0.5
            logger.info("Low quality, increased max_levels and cumulative threshold")
        
        elif quality > 0.9:
            # 很好，可以尝试提前停止，节省 token
            new_config["max_levels"] = max(
                current_config.get("max_levels", 3) - 1,
                2
            )
            new_config["cumulative_gain_threshold"] = max(
                current_config.get("cumulative_gain_threshold", 2.5) - 0.3,
                1.0
            )
    ...
    def ab_test(self, strategy_a: Dict, strategy_b: Dict, 
               query: str, evaluation) -> Dict:
        """A/B 测试两种策略"""
