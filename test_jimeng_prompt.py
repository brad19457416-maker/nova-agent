...
class MockLLMClient(LLMClient):
    ...
    def complete(self, prompt: str, **kwargs) -> str:
        ...
        elif "故事策划师" in prompt or ("page_number" in prompt and "content_summary" in prompt):
            # 阶段2：故事规划 - 12页
            pages_plan = []
            pages_data = [
                ("1", "咪咪在家里闷得慌，决定去森林探险"),
...
def main():
    # 使用 Mock LLM 测试完整流程，生成适配即梦的提示词
    print("🚀 初始化 Mock LLM (即梦适配)...")
    llm = MockLLMClient()
    
    # 创建生成器，指定 prompt_platform = "jimeng"
    ...
    output_file = os.path.join(output_dir, f"{book.title}_jimeng_prompts.txt")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for page in sorted(book.pages, key=lambda p: p.page_number):
            ...
            f.write(f"提示词(即梦): {page.image_prompt}\n\n")
    
    print(f"\n💾 完整提示词已保存到: {output_file}")
    print(f"\n✅ 即梦适配测试完成！总共 {len(book.pages)} 页，全部提示词已生成。")
    print("\n你可以直接复制这些提示词到即梦生成每一页的插图了。")
